...
prev_time = None
erase_radius = 70  # Increased from 30 to 50 for bigger eraser
color_palette = [
    (0, 255, 255),   # Yellow
    (0, 0, 255),     # Red
    (0, 255, 0),     # Green
...
three_finger_active = False  # Track if 3 fingers are currently up
color_changed_this_gesture = False  # Prevent multiple color changes
...
def is_back_of_hand(hand_landmarks):
    """Detect if back of hand is facing camera (palm facing away)."""
    # Use multiple indicators to detect back of hand
    
    # Method 1: Check if fingertips are "higher" (larger Y) than their base joints
    # When back of hand faces camera, fingertips appear behind knuckles
    index_tip = hand_landmarks.landmark[8]
    index_mcp = hand_landmarks.landmark[5]
    middle_tip = hand_landmarks.landmark[12]
    middle_mcp = hand_landmarks.landmark[9]
    
    # If tips are behind (higher Y) the base, it's likely back of hand
    fingers_behind = (index_tip.y > index_mcp.y) and (middle_tip.y > middle_mcp.y)
    
    # Method 2: Check thumb position relative to pinky
    # For right hand with back showing: thumb is on right side
    # For left hand with back showing: thumb is on left side
    thumb = hand_landmarks.landmark[4]
    pinky = hand_landmarks.landmark[20]
    wrist = hand_landmarks.landmark[0]
    
    # Calculate if thumb and pinky are on opposite sides of wrist
    thumb_side = thumb.x - wrist.x
    pinky_side = pinky.x - wrist.x
    
    # Back of hand: thumb and pinky are closer together (less spread)
    spread = abs(thumb.x - pinky.x)
    
    # Combine indicators: fingers appear behind knuckles AND hand spread is small
    return fingers_behind or spread < 0.15
    ...
    if result.multi_hand_landmarks:
        ...
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if index_up and not middle_up and not ring_up and not pinky_up:
                ...
                # Visual feedback with dynamic size (only if overlays enabled)
                if show_overlays:
            ...
            # 2️⃣ Erase Mode (All Fingers Open) - Use palm center for erasing
            elif all(fingers):
                if show_overlays:
                    cv2.putText(frame, "ERASER", (palm_x + 20, palm_y - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    cv2.circle(frame, (palm_x, palm_y), erase_radius, (0, 0, 255), 3)
                cv2.circle(canvas, (palm_x, palm_y), erase_radius, (0, 0, 0), -1)
                prev_x, prev_y = None, None
                prev_time = None
                points_buffer.clear()
            ...
            # 3️⃣ Change Color (Index + Middle + Ring Up)
                ...
                # Only change color once per gesture
                if not three_finger_active or not color_changed_this_gesture:
                    ...
                    color_changed_this_gesture = True
                ...
                # Reset color change flag when gesture is released
                if three_finger_active and not (index_up and middle_up and ring_up):
                    three_finger_active = False
                    color_changed_this_gesture = False
                
                prev_x, prev_y = None, None
                prev_time = None
                points_buffer.clear()

            # Draw skeleton (only if overlays are enabled)
            if show_overlays:
        ...
        if tracking_lost_frames <= max_lost_frames and last_known_position is not None and last_gesture_state is not None:
            ...
            index_x, index_y, palm_x, palm_y, fingers = last_known_position
            h, w, _ = frame.shape
            
            # Show warning indicator (only if overlays enabled)
            if show_overlays:
            ...
            if last_gesture_state == 'draw':
                ...
                # Visual feedback (only if overlays enabled)
                if show_overlays:
                    current_thickness = points_buffer[-1][2] if points_buffer else base_thickness
                ...
                if show_overlays:
                    cv2.putText(frame, "ERASER (tracking...)", (palm_x + 20, palm_y - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                    cv2.circle(frame, (palm_x, palm_y), erase_radius, (0, 0, 255), 2)
                cv2.circle(canvas, (palm_x, palm_y), erase_radius, (0, 0, 0), -1)
        else:
